...
> 三轴刻度：**Variance 4 / Motion 3 / Density 6**（Learn 表面经 `data-density="learn"` 降为 3，Do 表面维持 6）
...
**主题关键词**：暖米纸感 · 车间秩序 · 读/写分区 · 可操作

**氛围描述**
暖米底（`--bg #F3F3E9`）像摊开的教材而非后台系统，给零基础读者心理安全感。但同一套密度不能套所有表面——**双密度分区**：Learn 表面（主线 / 站页 / 章节 / 练习题面）Density 3，放宽间距、行宽、标题档，像教科书；Do 表面（SQL 沙箱 / 仿真 / 后台 / 表格）Density 6，保持工具密度，一屏塞尽量多信息。深色只出现在一个地方——SQL 编辑器面板，让"动手写"从"读"里切出来。学员打开页面第一眼看到的是「继续第 N 站」和下一步动作，不是标语。

**对标锚点**
工具面（Do）取三件、学习面（Learn）取四件，**只取结构，不取皮肤**。

| 面 | 锚点 | 取什么 |
|---|---|---|
| Do | **Linear** | 层级与克制的动效：150ms 状态切换、无入场编排、键盘可达 |
| Do | **Supabase Studio** | 浅色应用里嵌深色 SQL 工作区的分栏形态（核心页直接对标） |
| Do | **Stripe Docs** | 左目录 + 正文 + 右侧演示的三栏教学结构，理论与练习并置 |
| Learn | **Duolingo** | 单一线性路径 + 锁定/解锁的方向感 + 单元级「还差几件事」 |
| Learn | **Khan Academy** | 单元掌握度可视化、「接着上次学」的继续入口 |
| Learn | **Codecademy** | 三栏「讲解/动手/结果」+ checkpoint 即时判分 + 三级提示 |
| Learn | **Notion** | 长文呼吸感、折叠块、行内代码的低干扰处理 |

**寄存器判据**：主表面是工具（沙箱 / 列表 / 后台 / 表单），标杆是**赢得熟悉感**——一个用惯 Linear、Notion、Supabase 的人坐下来应当立刻会用。Learn 表面（主线 / 站页 / 章节）带轻 brand 成分：更大字号、更多留白、更强叙事感，但仍是工作台不是落地页；首页第一块是「继续第 N 站」，工厂全景降为其下方的地图。

**反目标（§7.1 已裁决：只取结构层游戏化，不取情绪层）**
- **取（结构层）**：单一线性路径（6 站主线）、锁定/解锁、即时判分、三级提示、「还差几件事」三点式微进度、方向感。
- **不取（情绪层）**：成就徽章雨、奖杯/星星、卡通吉祥物、连胜施压、排行、撒花/粒子/弹跳庆祝动画。**禁任何 celebration 类图标（trophy / medal / star / party）。**
...
| Token | 值 | 角色 |
|---|---|---|
| `--bg` | `#F3F3E9` | 页面底，暖米（纸/教材语义，本身即亲和） |
| `--surface` | `#FFFFFF` | 卡片 / 面板 |
| `--surface-2` | `#F7F7EF` | 表头 / 内嵌区 / hover 底（微米色） |
| `--surface-3` | `#EAEAE0` | 选中底 / 进度槽 / 按下态 |
| `--fg` | `#222222` | 主文本（深墨，非纯黑） |
| `--muted` | `#606864` | 说明文字，对 `--bg` ≈ 5.1:1 |
| `--border` | `#DDDFD6` | 卡片 / 输入框边框 |
| `--accent` | `#547C70` | **唯一强调色**（森林绿，与品牌墨色 `--brand-ink #2d3a33` 同源） |

`--accent` 对白 ≈ 4.6:1，白字压其上满足 WCAG AA（≥4.5:1）——既能当正文链接色，也能当按钮底色，不需要两套。每屏 ≤2 处可见使用。

**选色理由**：避开 Tailwind 默认 Indigo `#6366F1`（业界公认 AI 第一 tell）与全部紫/粉渐变。森林绿取自品牌墨色 `--brand-ink`（HMI/工控终端的墨绿数据色），落在制造业语境里且与深色 SQL 面板同源——不是第二套蓝。中性色为暖米色阶（带微量绿相 chroma，不是纯灰），与 accent 形成潜意识凝聚。暖米底比冷白更亲和，零基础读者第一眼读到的是「教材」而非「后台」。
...
### B-slot 别名
`--fg-2 #3d4540`（正文段落）· `--meta #8a928d`（时间戳/计数，仅 12–13px 非关键信息，对比度 ≈2.9:1，**不得承载正文或 UI 控件的唯一状态**）· `--border-soft #E8E9E0`（表格行分隔）· `--border-strong #c5c7bb`（输入框描边/拖拽条）

### C-extension（MES 专属）
- **SQL 编辑器深色面板**：`--code-bg #2d3a33`（与 `--brand-ink` 同值，墨色即面板）+ 6 个语法着色 token
...
| `--text-xl` | 20 | 区块标题 |
| `--text-2xl` | 24 | 页面标题 |
| `--text-3xl` | 30 | 工作台主标题（阶梯顶端，全站最大字号） |
...
- 字距：全大写标签 **0.08em**（硬性 ≥0.06em）；正文 0；≥24px 标题 −0.011em；30px 主标题 −0.021em（阶梯顶端，最紧字距）。
- 表格与所有数字：`font-variant-numeric: tabular-nums`，列对齐不跳。

### 3.5 真实业务记录内联样式（"真实记录"样式）

章节正文会**频繁**出现真实单号、表名、数量/比例（如 `WO-20260801-02`、`work_order`、计划 60 / 已完成 40）。这些不是散文，是「系统里的一条真实记录」，需用排版与散文区分——**不靠颜色**（不占 accent 配额），靠 `--font-mono` + 中性色阶。

| 元素 | 示例 | 样式 |
|---|---|---|
| 单据号 | `WO-20260801-02` | `--font-mono` + `--fg` + `tabular-nums`；需强提示时加 `--surface-2` 微底 + `--radius-xs`，不强制 |
| 表名 | `work_order` | `--font-mono` + `--fg-2` + 1px `--border-soft` 描边（行内 code 形态） |
| 数字/比例 | `计划 60 / 已完成 40` | `--font-mono` + `tabular-nums`，列对齐不跳 |

规则：
- 复用既有 `--font-mono`（JetBrains Mono），**不新增字体 token**；行内 code 与「真实记录」共用一套。
- 密度高，默认**不加背景块**（避免花），仅等宽 + 字色；只在「这条是样例真实记录」需强提示时才加 `--surface-2` 微底。
- 不靠颜色单独传意：单据号旁必要时配 8px 状态点 + 文字（沿用主线单据卡规则）。
...
### 5.4 双密度分区与进度（Beginner Redesign 锁定项）

**双密度分区**：Learn 表面 Density 3 / Do 表面 Density 6，由 `design-tokens.css` 的 `[data-density='learn']` 覆写块实现（覆写 `--card-padding` / `--card-title-size` / `--section-y-*` / `--prose-*` 等既有 B-slot token，**不新增 token、不改 A1**）。Learn 路由布局容器挂 `data-density="learn"`；练习页是唯一例外——题面栏挂 learn、编辑器/结果栏不挂（「读」与「写」视觉分离）。

**进度显示（PM F10 锁定）**：
- 站内：用「知识 / 练习 / 测验」三点式微进度（3 个 8px 圆点，完成填充）——可数的、可完成的，不用百分比。
- 全站总进度：「当前阶段完成度 + 总进度 **N/12**」。N/12 的 12 取自工厂全景 12 节点（FactoryFlow v10）；6 站主线是面向学员的叙事分组。**节点→站点映射待用户阶段决策后定稿**。
- N/12 主进度条全站只出现一处（顶栏下方或主线阶梯顶部，二选一，不重复）。

**阶段划分暂缓（PM 更新 #2）**：tour/basics/deep 划分待用户决策。全景图节点须同时支持两种布局，先不定稿：
- 具象优先（纯计算节点如 MPS/MRP 靠后）→ 出现节点跳跃，跳过节点须**弱化但可见**（沿用 `--state-disabled-opacity` + `--meta` 文字，不隐藏），并配一句明示「此处跳过」。
- 业务流顺序 → 无跳跃，不适用。
...
- 对比度：正文 ≥4.5:1 / UI 组件 ≥3:1。`--muted` ≈5.1:1（正文级）、`--accent` 白字压其上 ≈4.6:1（AA）；`--meta` ≈2.9:1，**仅限 12–13px 非关键信息，不得承载正文或 UI 控件的唯一状态**。精确比值由 Phase 3 a11y 审计专项验收。
...
| 2026-08-08 | §1/§2/§3.4 对齐零基础重学方案：反目标精确为「反情绪层游戏化」（只取结构层，team-lead §7.1 裁决）；A1-identity 同步森林绿 v3（`#547C70`/`#F3F3E9`/`#222222` 及 surface/muted/border 派生，修掉滞留的 v1 工程青 `#0E7490`/`#F7F9FB`/`#0F1B26`）；删除 `--text-4xl`、字距顶端对齐 30px、新增双密度分区说明；§8 对比度预验值更新为 v3 实测 | 用户反馈「知识不好理解」→ 零基础重学方案落地；DESIGN.md 色板与 `design-tokens.css` 唯一事实源对齐 | §1 / §2 / §3 / §8 |
| 2026-08-08 | §2 C-extension `--code-bg` 修正 `#0F1B26` → `#2d3a33`（与 v3 `--brand-ink` 同值，修掉 v1 残留） | 避免前端照 §2 实现时渲染出第二套深色面板色 | §2 |
| 2026-08-09 | **§1/§2/§3 对齐森林绿 v3 + 零基础重学锁定项**：§2 三 A1 token 及全色板改 `#F3F3E9`/`#222222`/`#547C70`（废弃工程青 `#0E7490`/`#F7F9FB`/`#0F1B26`）；§1 反目标精确为「反情绪层游戏化，只取结构层」+ 双密度 Learn/Do + Learn 对标（Duolingo/Khan/Codecademy/Notion）；删 `--text-4xl`、修 `--tracking-display` 至 30px；新增 §3.5 真实记录内联样式、§5.4 双密度分区 + N/12 进度 + 阶段暂缓 | §7.1/§7.2 裁决落地 + PM F10 进度锁 + 「不要比喻」裁决 | 全站色板 / 零基础 Learn 表面 |
