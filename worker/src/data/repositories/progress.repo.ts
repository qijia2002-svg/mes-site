...
/**
 * 进度数据访问。匿名身份 anon_id 由前端 localStorage 生成并全链路透传（F4）。
...
export const progressRepo = {
   ...
   */
  bumpStats(db: DbSession, anonId: string, col: keyof StatsDailyRow): Promise<D1Result> {
    const ts = Date.now();
    return db.run(
  ...
  listByAnon: (db: DbSession, anonId: string, limit = 50) =>
  ...
  summaryByAnon: (db: DbSession, anonId: string) =>
  ...
  completedItems: (db: DbSession, anonId: string, itemType: ItemType) =>
  ...
  todayStats: (db: DbSession, anonId: string) =>
