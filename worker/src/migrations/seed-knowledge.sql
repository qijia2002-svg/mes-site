...
## 奥来现场对应
你在信息化专员岗位上用过 ERP。奥来的 ERP 主要用于：
- 数据采集
- 制作生产大屏

**观察任务：** 看看奥来的 ERP 系统里有哪些模块在用？哪些没用？
...
## KBO场景举例
ERP：今天要生产1000个KBO，需要多少物料？
MES：这1000个KBO经过15道工序，每道工序质量如何？不良率多少？
...
## 奥来AS-IS
现在：线长在微信群里发"今天做1000个KBO"，没有电子工单。
...
WHERE p.sn = ''KBO-20260719-001'';
```

## 为什么要会 JOIN？
面试官问："这个断路器用了哪个供应商的线圈、经过哪些工序、谁做的、测试结果？"
...
> 启示：PLC + SCADA 是重工业自动化的标配，KBO 漏保产线虽小，但"PLC 控制 + SCADA 监控 + 数据落库"的架构完全一致。
